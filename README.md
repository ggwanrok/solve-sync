...
기존 데이터베이스를 업데이트하는 경우 필요한 기능 SQL을 적용한 다음 `supabase/security-hardening.sql`을 마지막에 실행합니다. 스터디룸 디렉터리 페이지네이션을 추가하려면 `supabase/study-room-directory-pagination.sql`을 실행합니다.

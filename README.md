...
`VAPID_SUBJECT`에는 운영자 이메일을 `mailto:` 형식으로 입력합니다. `CRON_SECRET`에는 충분히 긴 임의 문자열을 넣으면 Vercel이 `vercel.json`에 등록된 예약 작업을 호출할 때 인증 헤더로 전달합니다. 예약 작업은 한국 시간 기준으로 매일 오후 6시에 미달성 목표 알림을 보내고, 매일 오전 6시에 일간 스터디의 전날 브리핑을 전달합니다. 주간 스터디 알림은 일요일 오후 6시, 지난주 브리핑은 월요일 오전 6시에만 전달합니다.

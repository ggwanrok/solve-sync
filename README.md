...
**내 친구 → 순서 편집**에서 손잡이를 드래그하거나 화살표 버튼으로 원하는 순서를 정한 뒤 **순서 저장**을 누릅니다. 손잡이에 키보드 초점을 둔 상태에서도 위·아래 방향키로 이동할 수 있습니다. 순서는 내 계정에 저장되어 다른 기기에도 적용되며, 새 친구는 목록 끝에 추가됩니다. **취소**하면 저장 전 순서로 돌아갑니다.
...
방장이 스터디룸 이름·설명·공개 여부를 변경할 수 있게 하려면 기존 데이터베이스에 `supabase/study-room-settings.sql`을 실행합니다. 공개 방을 비공개로 바꿀 때만 새 입장 비밀번호가 필요하며, 비공개 방을 공개로 바꾸면 기존 비밀번호와 인증 기록이 폐기됩니다. 새 설치용 `schema.sql`에도 포함되어 있습니다.

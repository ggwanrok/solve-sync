...
import { ProblemBookmarks } from "@/components/problem-bookmarks"
  ...
  return (
    <div className="page-container-wide">
      ...
      <Tabs value={tab} onValueChange={(value) => {
        ...
        <TabsList aria-label="문제 보기">
          <TabsTrigger value="notes"><NotebookPen />문제 메모</TabsTrigger>
          <TabsTrigger value="bookmarks"><Bookmark />북마크</TabsTrigger>
        </TabsList>
            ...
            <div className="grid min-w-0 items-start gap-5 md:grid-cols-[16rem_minmax(0,1fr)]">
              <Card className={cn("min-w-0 md:sticky md:top-21", mobileEditorOpen && "hidden md:flex")}>
                <CardHeader className="border-b">
                  ...
                  <div role="group" aria-label="문제 메모 필터" className="mt-2 grid grid-cols-3 gap-1 rounded-xl bg-muted/70 p-1">
        ...
        <TabsContent value="bookmarks" keepMounted>
          <ProblemBookmarks
            problems={problems}
            pendingIds={reviews.pendingIds}
            onToggle={reviews.toggle}
            onOpenMemo={(problemId) => {
              setTab("notes")
              setFilter("all")
              setSearch("")

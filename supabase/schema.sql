...
begin
  ...
  if notification_phase = 'reminder' then
    ...
    from (
      ...
      cross join lateral (
        select date_trunc(case when room.goal_period = 'daily' then 'day' else 'week' end, local_run_at) at time zone 'Asia/Seoul' as period_start,
    ...
    from (
      ...
      where (room.goal_period = 'daily'
        ...
        and member.joined_at < period.period_end and room.created_at < period.period_end
    ) candidate
...
begin
  ...
  if room_is_private is null then raise exception '공개 설정을 확인해 주세요.' using errcode = '22023'; end if;

  select * into target_room from public.study_rooms

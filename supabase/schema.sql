...
create table if not exists public.study_notifications (
  ...
  type text not null check (type in ('goal_reminder', 'goal_missed', 'weekly_summary', 'poke')),
...
returns table(id uuid, study_id uuid, type text, title text, body text, url text, created_at timestamptz, read_at timestamptz)
language plpgsql stable security definer set search_path = '' as $$
begin
  if auth.uid() is null then raise exception '로그인이 필요합니다.' using errcode = '42501'; end if;
  return query
  select notification.id, notification.study_id, notification.type, notification.title,
    notification.body, notification.url, notification.created_at, notification.read_at
  from public.study_notifications notification
...
begin
  if auth.uid() is null then raise exception '로그인이 필요합니다.' using errcode = '42501'; end if;
  update public.study_notifications notification set read_at = now()
  where notification.recipient_id = auth.uid() and notification.read_at is null
...
returns table(user_id uuid, handle text, nickname text, avatar_url text, solved_count bigint)
language plpgsql stable security definer set search_path = '' as $$
begin
  if auth.uid() is null then raise exception '로그인이 필요합니다.' using errcode = '42501'; end if;
  if not exists(select 1 from public.study_members member where member.study_id = target_study and member.user_id = auth.uid()) then return; end if;
  return query
...
begin
  ...
  from (
    ...
    cross join lateral (
      select date_trunc(case when room.goal_period = 'daily' then 'day' else 'week' end, now() at time zone 'Asia/Seoul') at time zone 'Asia/Seoul' as period_start,
    ...
    left join lateral (
      select count(distinct event.problem_id)::bigint as solved_count
      from public.solve_events event
      where event.user_id = member.user_id and coalesce(event.difficulty, 0) >= room.min_difficulty
        and event.accepted_at >= greatest(period.period_start, member.joined_at) and event.accepted_at < period.period_end
    ) progress on true
    where member.joined_at < period.period_end and room.created_at < period.period_end
  ) candidate
  where now() >= candidate.period_end - interval '6 hours' and now() < candidate.period_end
  ...
  from (
    ...
    join public.study_members member on member.study_id = room.id
    cross join lateral (
      select (date_trunc(case when room.goal_period = 'daily' then 'day' else 'week' end, now() at time zone 'Asia/Seoul')
  ...
  cross join lateral (
    select (date_trunc('week', now() at time zone 'Asia/Seoul') - interval '1 week') at time zone 'Asia/Seoul' as period_start,
  ...
  where room.goal_period = 'weekly' and room.created_at < period.period_end
  ...
  where notification.type in ('goal_reminder', 'goal_missed', 'weekly_summary')
    and notification.pushed_at is null and notification.created_at >= now() - interval '8 days'
    ...
    and exists(select 1 from public.study_members member where member.study_id = notification.study_id
      and member.user_id = notification.recipient_id and member.notifications_enabled)
    and exists(select 1 from public.push_subscriptions subscription where subscription.user_id = notification.recipient_id)

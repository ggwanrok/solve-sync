      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          ...
          <CardContent className="grid gap-3 sm:grid-cols-2">
            {friends.length ? friends.map((friend) => <FriendCard key={friend.id} friend={friend} contributions={contributionsByFriend.get(friend.id) || []} />) : <p className="col-span-full py-10 text-center text-sm text-muted-foreground">아직 친구가 없어요. @아이디로 첫 친구를 추가해보세요.</p>}
          </CardContent>
        </Card>
        <Card>

      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          ...
          <CardContent className="grid gap-3 sm:grid-cols-2">
            {friends.length ? friends.map((friend) => <div key={friend.id} className="flex items-center gap-3 rounded-xl border p-3"><UserAvatar name={friend.nickname || friend.handle} className="size-11" /><div><p className="text-sm font-medium">{friend.nickname || friend.handle}</p><p className="text-xs text-primary">{friend.handle}</p></div></div>) : <p className="col-span-full py-10 text-center text-sm text-muted-foreground">아직 친구가 없어요. 닉네임으로 첫 친구를 추가해보세요.</p>}
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent className="flex flex-col gap-3">
            {requests.length ? requests.map(({ id, profile }) => <div key={id} className="flex items-center gap-3"><UserAvatar name={profile.nickname || profile.handle} className="size-10" /><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">{profile.nickname || profile.handle}</p><p className="text-xs text-muted-foreground">{profile.handle}</p></div><form action={respondFriendRequest} className="flex gap-1.5"><input type="hidden" name="requestId" value={id} /><Button type="submit" name="decision" value="accept" size="icon" className="size-8" aria-label="수락"><Check className="size-4" /></Button><Button type="submit" name="decision" value="decline" size="icon" variant="outline" className="size-8" aria-label="거절"><X className="size-4" /></Button></form></div>) : <p className="py-4 text-center text-sm text-muted-foreground">받은 친구 요청이 없어요.</p>}
          </CardContent>
        </Card>
      </div>
    </div>

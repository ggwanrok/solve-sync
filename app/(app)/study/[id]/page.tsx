...
export default async function StudyDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="mx-auto flex max-w-6xl flex-col gap-6">
      ...
      <Card><CardContent className="p-6"><div className="flex flex-col gap-4 lg:flex-row lg:items-center"><div className="min-w-0 lg:max-w-[42%]"><div className="flex items-center gap-2"><h1 className="truncate text-xl font-bold">{room.name}</h1>{room.ownerId === user?.id && <Badge variant="secondary" className="gap-1"><Crown className="size-3" />리더</Badge>}</div><p className="mt-1 text-sm text-muted-foreground">{room.description}</p><div className="mt-3 flex flex-wrap gap-2 text-sm"><span className="rounded-lg bg-primary/10 px-2.5 py-1 font-medium text-primary">{room.goalPeriod === "daily" ? "매일" : "매주"} {room.goalCount}문제</span><span className="rounded-lg bg-muted px-2.5 py-1 font-medium">Lv.{room.minDifficulty} 이상</span><span className="flex items-center gap-1 text-muted-foreground"><Users className="size-4" />{detail.members.length}명</span></div></div><StudyDailyChampionBanner champions={dailyChampions} className="w-full lg:ml-2 lg:w-auto lg:min-w-72" /><div className="flex shrink-0 flex-col items-start gap-2 lg:ml-auto lg:items-end">{isMember && <StudyNotificationToggle studyId={id} initialEnabled={currentUserNotificationsEnabled} />}{!isMember && <JoinStudyRoomButton studyId={id} />}</div></div></CardContent></Card>

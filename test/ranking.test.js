...
test("알고리즘과 SQL 점수를 독립적으로 산출하고 SQL 점수의 절반을 반영한다", async () => {
  ...
  assert.equal(breakdown.algorithmScore, 31)
  assert.equal(breakdown.sqlScore, 6)
  assert.equal(breakdown.rankingScore, 34)
...
test("SQL 점수는 2로 정수 나눗셈해 소수점을 버린다", async () => {
  ...
  assert.equal(breakdown.sqlScore, 21)
  assert.equal(breakdown.rankingScore, 10)
})
...
test("상위 100문제 제한은 알고리즘과 SQL에 각각 적용한다", async () => {
  ...
  assert.equal(breakdown.algorithm.topProblemScore, 3_000)
  assert.equal(breakdown.sql.topProblemScore, 3_000)
})
